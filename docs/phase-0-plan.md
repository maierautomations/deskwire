...
- **Vorbedingungen (dokumentieren):** Stripe CLI ohne Homebrew: macOS-arm64-Binary von den offiziellen GitHub-Releases (stripe/stripe-cli) laden, entpacken, nach `~/bin` verschieben und PATH prüfen. Alternative: offizielles Docker-Image stripe/stripe-cli, für `stripe listen` dann mit `--forward-to host.docker.internal:3000`. Danach `stripe login` mit Testmode-Account; Produkt + Preis im Testmode-Dashboard; Test-Customer anlegen und `stripe_customer_id` am Workspace eintragen (Drizzle Studio — dokumentierter Ops-Schritt, da keine Checkout-UI existiert); Dashboard-Webhook-Endpoint auf die Prod-URL mit den drei Subscription-Events, **API-Version des Endpoints auf die vom SDK gepinnte Dahlia-Version**; `STRIPE_SECRET_KEY` + `STRIPE_WEBHOOK_SECRET` in Vercel Production.
...
10. **`Redis.fromEnv()` wirft synchron bei fehlender Env** → Limiter lazy in try/catch, sonst stirbt der Magic-Link-Versand genau an dem Schutz, der ihn absichern soll (Task 9). **Task-9-Befund:** In @upstash/redis 1.38.0 wirft `fromEnv()` nicht mehr, es warnt nur und liefert einen Client, der erst beim Request stirbt — die Env-Präsenz wird deshalb explizit geprüft, das try/catch bleibt als zweite Sicherung.
