...
function termList() {
  return z
    ...
    .pipe(
      z
        .array(z.string().max(BRAND_PROFILE_LIMITS.begriff))
...
const pflichtelementSchema = z.strictObject({
...
const formatregelnSchema = z
  .strictObject({
    max_kicker_zeichen: zeichenLimit(),
    max_titel_zeichen: zeichenLimit(),
    max_seo_titel_zeichen: zeichenLimit(),
    max_teaser_zeichen: zeichenLimit(),
...
const beispieltexteSchema = z
  ...
  .pipe(
    z
      .array(z.string().max(BRAND_PROFILE_LIMITS.beispieltext))
...
export type Pflichtelement = z.infer<typeof pflichtelementSchema>;
